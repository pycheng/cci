...
class bst{
    public:
        ...
        //return in-order successor with parent link
        node *successor(node *n){
            ...
            if(succ){
                //case 1 find leftmost child of right subtree
                while(succ->left) succ = succ->left;
            }else{
                //case 2 find the first parent with left subtree contains n
                succ = n->parent;
                node *cur = n;
                while(succ && succ->left != cur){
                    cur = succ;
                    succ = cur->parent;
                }
            }
...
int main(){
    ...
    build_bst(num, t);
    t.dump();
    cout << t.successor(t.find(8))->v << "\n";  //9
    t.del(9);
    cout << t.successor(t.find(8))->v << "\n";  //10
    cout << t.successor(t.find(10))->v << "\n"; //11
    cout << t.successor(t.find(1))->v << "\n";  //2
    cout << t.successor(t.find(15)) << "\n";    //NULL
}
